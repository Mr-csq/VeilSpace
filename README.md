...
- 项目仅支持 Android 16 / API 36 及以上版本。
- `minSdk`、`targetSdk` 和 `compileSdk` 统一为 36。
- 后续开发只维护和测试 Android 16+，不再增加 Android 15 及以下版本的条件分支、旧 API 回退或旧权限声明。
- 如需降低最低版本，必须先重新评估 Work Profile、MediaStore、前台服务、通知权限、精确闹钟和跨资料 Intent 的完整兼容矩阵。
...
- 开始边界：为支持的所选应用启用 VeilSpace `keepAlive`、取消隐藏，并在 Profile Owner 和应用声明均满足时授予 `POST_NOTIFICATIONS`。
...
Android 16 上优先使用 `AlarmManager` 精确闹钟。未授权时会安全降级为可能延迟的闹钟，并在界面显示状态和授权入口。开机、应用升级、系统时间/时区变化及工作资料恢复后会重新计算；只有尚未执行的最新边界会补偿一次。
...
环境要求：JDK 17、Android SDK 36、Android Gradle Plugin 8.10.1、Gradle 8.11.1。仓库已包含 Wrapper，Windows 下使用：

```powershell
.\gradlew.bat --no-daemon --max-workers=1 --console=plain testDebugUnitTest
.\gradlew.bat --no-daemon --max-workers=1 --console=plain lintDebug
...
2026-07-17 Android 16 基线离线复核：Debug APK 构建通过且产物为 36/36/36；29 个 JVM 测试全部通过；lint 为 0 个错误、177 个警告。最终配置使用 AGP 8.10.1 / Gradle 8.11.1；当前环境无法下载该工具链，本次源码验证使用本机缓存工具链完成。

## 设备限制

- Profile Owner、跨资料能力和系统应用支持程度会受 OEM 定制影响，MIUI / HyperOS 上尤其明显。
- Android 16 上的目标应用通知权限只有在应用声明 `POST_NOTIFICATIONS` 且 DPM 接受操作时才会记录为成功；单个应用失败不会阻断其他应用。
