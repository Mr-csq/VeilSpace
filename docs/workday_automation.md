...
“停用下一次工作时段”会绑定到下一次开始边界的工作日期，而不是保存一个无目标的布尔值。目标日期的开始与配对结束边界都会被跳过，期间不会更改所选应用的 `keepAlive` 或通知权限；再后一个工作日恢复正常执行。

- 开始边界被跳过后，界面开关立即复位为关闭，但内部记录会保留到配对结束边界，防止结束策略误执行。
- 页面保持前台时会在下一边界后主动刷新；跨重启、开机恢复或延迟唤醒时也会从持久化状态恢复。
- 尚未开始时修改计划，会把停用目标重新绑定到新计划的下一次开始；已经开始后只保护当前工作时段的结束边界。
- 若应用直到目标时段之后才恢复，过期记录会被清除，不会跳过后续工作日。
- 边界执行结果与停用状态在同一次持久化提交中更新，重复广播不会重复消费。
